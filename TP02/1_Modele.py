...
def feature_extraction(input):
    ...
    encoded = MaxPooling2D((2, 2), padding="same")(
        x
    )  # L'ensemble des features/caractéristiques extraits
...
training_data_generator = ImageDataGenerator(
    rescale=1.0 / 255, shear_range=0.1, zoom_range=0.1, horizontal_flip=True
)
...
training_generator = training_data_generator.flow_from_directory(
    ...
    shuffle=True,
)  # on "brasse" (shuffle) les données -> pour prévenir le surapprentissage
